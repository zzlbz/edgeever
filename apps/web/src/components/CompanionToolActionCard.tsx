  ...
  return (
    <div className="space-y-3 pt-0.5" aria-label={title}>
      <div className="space-y-1">
        ...
        <p className="line-clamp-3 whitespace-pre-wrap break-words text-xs leading-relaxed text-slate-600 ">{action.plan.reason}</p>
      </div>
      {action.status === "pending" ? (
        <div className="space-y-2.5">
          ...
          {action.notes.length > 0 ? (
            <div className="space-y-1.5 rounded-lg border border-slate-200/70 bg-slate-50/70 p-2.5  ">
          ...
          {special ? (
            <p className="rounded-lg border border-rose-200/60 bg-rose-50/60 p-2.5 text-xs text-rose-600   ">
